...
for scale in SCALES:
    ...
    for name in WLS:
        ...
        def add(method, budget, fr, ne, t, seed=None):
            rows.append(dict(scale=scale, workload=name, method=method, budget=budget,
        ...
        for B in (100, 250, 500, 1000):
            for s in SEEDS:
                r = random_search(sp, wl, B, seed=s); add("random", B, r.front, r.n_evaluated, r.runtime, s)
                g = nsga2(sp, wl, B, seed=s);        add("nsga2",  B, g.front, g.n_evaluated, g.runtime, s)
