...
def write(n,s):(G/(n+'.tex')).write_text(s+'\n')
def ran(a,d=0):
...
write('rq1',r'Table~\ref{tab:funnel} and Fig.~\ref{fig:funnel} report the exploration funnel. On S3 ($51\,840$ candidates), HCA-DSE performs $\SthreeEvalRange$ full evaluations, removing '+ran((100*(1-int(r['evals'])/int(r['raw'])) for r in s3),2)+r'\% of the raw space before evaluation. On S4 ($345\,600$ candidates), it performs $\SfourEvalRange$ evaluations, a reduction of '+ran((100*(1-int(r['evals'])/int(r['raw'])) for r in s4),2)+r'\%. Raw-space reductions include structural rejection; Table~\ref{tab:funnel} separately reports the smaller number of actual exhaustive evaluator calls.')
lines=[]
for sc in ['S2','S3']:
...
Fig.~\ref{fig:crossover} is a cost projection from measured counts and median overheads;
...
write('order',r'Table~\ref{tab:order} crosses order and queue-bound activation on S3 (all scales in the supplement). With the queue bound enabled, O1 performs $'+ran(int(r['evals']) for r in d if r['order']=='O1')+r'$ full evaluations and O2 performs $'+ran(int(r['evals']) for r in d if r['order']=='O2')+r'$. O2 has $'+ran(int(r['partial_queue_active']) for r in d if r['order']=='O2')+r'$ checked incomplete prefixes with a positive queueing contribution, of which $'+ran(int(r['partial_queue_pruned']) for r in d if r['order']=='O2')+r'$ are pruned only because that term is present against the same archive. All 36 scale/workload/order/queue combinations preserve the unrounded exhaustive objective-vector set. Changes between O1 and O2 also change the archive discovery sequence; only the within-order on/off comparison isolates the bound toggle.')
...
write('validation',r'''Table~\ref{tab:des} reports analytical mean-latency predictions against the revised
DES model. The independent Docker round retains the same 36 configurations and all 108
runs with the analytical model frozen. Table~\ref{tab:testbed} compares predictions with
