...
class Bounder:
    def __init__(self, space: DesignSpace, wl, use_queue_bound=True):
        self.use_queue_bound = use_queue_bound
    ...
    def bounds(self, partial: dict) -> Dict[str, float]:
        ...
        if c is None:
            ...
            self._comm[key] = c
        L += c
        if self.use_queue_bound:
            L += _queue_bound(partial, self.space, wl)
