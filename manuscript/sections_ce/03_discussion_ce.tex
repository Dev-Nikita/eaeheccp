\section*{Discussion}
\label{sec:ce:discussion}

The practical obstacle to architecture exploration for distributed cyber-physical systems
is the cost of an answer about one candidate, not the number of candidates. The results
above change what that cost buys. Reducing the expensive evaluations from the size of the
...
What makes this possible is narrower than "pruning works", and worth stating precisely.
...
studied here. It applies wherever an objective decomposes into non-negative terms that
are individually monotone in the free variables and where a subset of decisions
determines a load-dependent term --- which covers service placement in a computing
continuum, sensing and control pipelines in robotics, and industrial edge deployments
with mixed hardware tiers. Demonstrating that transfer on a structurally different
...
Three limits are worth stating plainly. First, when the evaluator is cheap the pruning
