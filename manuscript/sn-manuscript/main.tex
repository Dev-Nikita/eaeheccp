...
costs. Candidate architectures number in the millions and simulating one can take up to a
...
on objectives that grow monotonically as decisions are fixed. End-to-end latency does not,
because it depends on the utilisation of a tier that several decisions fix jointly. We
...
provably safe elimination of infeasible and dominated regions before any evaluation. On
...
with the model frozen beforehand, predicted and measured orders of two architectures
agreed for \WithinAgreementRange\% of pairs separated by more than 25\% and for
\ReliabilityFifty{} pairs separated by more than 50\%. Analytical models can thus
certify which architectures need no high-fidelity evaluation.}
...
method is therefore best read as a certified screening stage that decides which
architectures need never reach a simulator or a test bench. We report equally clearly where the gains shrink or the method does not apply: the
...
Agreement grows with the separation the model predicts (\RefReliabilityFig{}): 86\% of all 196 within-workload pairs are ordered as predicted, 90\% of the 105 pairs the model separates by more than $25\%$, 53 of 54 pairs separated by more than $50\%$ and 20 of 20 separated by more than $100\%$.
...
$95\%$ Wilson interval; dashed line: only architectures whose repeats agree within a
factor of $1.5$.}
