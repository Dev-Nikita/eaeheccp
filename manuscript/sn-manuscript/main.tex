...
%
The scalability sweep (\RefScalSite{}) covers raw spaces from $1.3\times10^3$ to $8.3\times10^7$ candidates. Across the sweep, expanded partial states never exceed $16780$; full evaluations range from $18\text{--}61$. The largest observed runtime is $3.98$\,s, measured with Python memory tracing enabled, and peak traced allocations are $99.6$\,KiB. This is traced Python memory during search, not process RSS or total interpreter memory. Exhaustive front equality is not checked for these larger spaces.
...
%
The layer ablation on S3 (\RefAblationSite{}) separates the contributions of the three pruning layers. Structural pruning alone leaves $34\,560$ full evaluations. Structural plus feasibility bounds leave $7025\text{--}12700$, while structural plus dominance bounds leave $322\text{--}415$. Combining all layers leaves $\SthreeEvalRange$. Every variant preserves the exhaustive analytical front; the separate layers contribute complementary reductions on these spaces.
...
Large language model assistants were used to help draft and revise manuscript text and to
assist with software implementation. All text, code and results were checked and edited by
the authors, who take full responsibility for the content; no AI tool is listed as an
author, and no AI tool generated or altered experimental data.
