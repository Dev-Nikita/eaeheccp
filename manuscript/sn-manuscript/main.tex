...
%% ORCID: N. Tarasov 0000-0003-1870-3436; V. Tomyuk 0000-0003-4999-7019
...
specified design already carries a valid lower bound on all its completions. The dominant objective in a distributed cyber-physical
...
within a workload. We report equally clearly where the approach does not help: when the evaluator
...
On S3 at $B=500/1000$, 4 of the six exploratory one-sided comparisons have unadjusted $p<0.05$; $p$ ranges from $0.000\text{--}0.168$ and Cliff\textquotesingle{}s $\delta$ from $0.180\text{--}0.800$. The individual results are in \RefStatsTab{}; no correction for multiple comparisons is applied. HCA-DSE recovers the full analytical front on S3 with $\SthreeEvalRange$ evaluations.
...
Three limits apply to the results reported here. First, when the evaluator is cheap the pruning
overhead is comparable to the evaluation it avoids and the wall-clock benefit largely
disappears --- the measured speed-up with the closed-form evaluator is $\AnalyticalSpeedup\times$
...
deployment, ran the experiments and wrote the manuscript. V.T. (ORCID
0000-0003-4999-7019) contributed to the system model and the experimental design and
