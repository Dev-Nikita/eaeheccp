...
Exact exploration methods that prune partial solutions assume that the objectives are
...
optimistic but valid. Every term of \eqref{eq:latency}--\eqref{eq:netvol} is monotone in
each of its free variables --- service time decreases in $C$, transfer time decreases in
$B$, static energy and cost increase in the node count, network volume increases in $r$
--- so each $\underline{\varphi}_t(p)$ is attained at a domain endpoint and is obtained
from a table of precomputed domain extrema in $O(1)$ time rather than by search.
...
\label{rem:order}
The order $v_1,\dots,v_k$ places the determinants of tier utilisation --- node counts,
node classes, replication factor and policy --- before the remaining variables. As soon
as they are assigned, the waiting term
\begin{equation*}
W_t=\frac{w_i}{C_t}\cdot\frac{\rho_t}{2\,(1-\rho_t)}
\end{equation*}
is constant on $\C(p)$ and enters the bound
\emph{exactly} by Lemma~\ref{lem:exact}, instead of being relaxed to its trivial bound
$0$. This is what makes a contention-dependent objective usable for safe pruning without
assignment monotonicity. If the determinants are fixed and $\rho_t\ge1$, no completion has
finite latency and $\LB_L(p)=\infty$, which is admissible.
\end{remark}
...
\end{theorem}
\begin{proof}
$(\subseteq)$ Every returned vector is $F(x)$ of an evaluated $x\in\F_\sigma$ and survives
non-dominated filtering; a dominating architecture would itself have been evaluated or
pruned only by Proposition~\ref{prop:dom}, which is excluded by that proposition.
...
objective vector, dominance pruning may remove one of them; the front is unaffected but
the duplicate solution is lost. All experiments therefore compare sets of objective
vectors.
\end{remark}
...
Let $C_E$ be the cost of one full evaluation, $C_P$ the cost of computing the bounds at
one tree node, $N$ the number of visited partial states, $N_E$ the number of full
evaluations and $|A|$ the archive size. Then
\begin{equation}
T_{\mathrm{exh}}=O\big(|\X|\,C_E\big),\qquad
T_{\mathrm{HCA}}=O\big(N(C_P+n|A|)+N_E\,C_E\big),
\label{eq:complexity}
\end{equation}
and pruning is profitable when
\begin{equation}
C_E \;>\; \frac{N\,(C_P+n|A|)}{|\X|-N_E}.
\label{eq:breakeven}
\end{equation}
Section~\ref{sec:results} measures the right-hand side and shows that it lies in the
microsecond range, and that $N\ll|\X|$, so the Cartesian product is never materialised.
