...
\paragraph{Latency}
We model a tier as a single queueing station whose service rate is the \emph{aggregate}
capacity $N_tC_t$ of its nodes, with deterministic service and Poisson arrivals, and use
the Pollaczek--Khinchine formula for M/D/1~\cite{kleinrock1975queueing,
harchol2013performance} for the waiting time. This is an approximation in two respects:
a tier of $N_t$ single-server nodes with per-request dispatching is an M/D/$c$ system
rather than an M/D/1 system with $c$-fold capacity, and the approximation is optimistic
because it lets one request use the capacity of the whole tier. We keep it deliberately,
for three reasons: it is the coarsest model that still makes latency depend on
utilisation, which is what the bounds of Section~\ref{sec:method} must handle; it is
monotone in the design variables, which is what makes those bounds computable in closed
form; and its consequences are quantified rather than assumed --- both the discrete-event
evaluator and the testbed of Section~\ref{sec:testbed} implement per-node servers, so
the residual error of the approximation is measured in Section~\ref{sec:results} rather
than argued away. With
